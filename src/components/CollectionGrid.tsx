...
export function CollectionGrid({ onImport }: CollectionGridProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return items;
  }, [ratings, debouncedSearch, typeFilter, sortBy]);
  ...
  return (
    <div>
      ...
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <h2 className="font-display text-xl text-foreground">
          {debouncedSearch || typeFilter !== "all"
            ? `${filtered.length} de ${ratings.filter((r) => r.tmdbId !== null).length} títulos`
      ...
      <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4">
        {filtered.slice(0, visibleCount).map((rating, index) => (
          <motion.div
            key={`${rating.tmdbId}-${rating.tmdbType}-${index}`}
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-50px" }}
            transition={{
              duration: 0.4,
              delay: Math.min((index % 48) * 0.03, 0.3),
              ease: "easeOut",
            }}
          >
            <TitleCard
      ...
      {visibleCount < filtered.length && (
        <div className="mt-10 text-center">
          <button
            ...
            className="focus-ring inline-flex items-center gap-2 rounded-full bg-background-subtle px-6 py-2.5 text-sm font-medium text-foreground-muted transition-colors duration-200 hover:bg-border hover:text-foreground"
          >
